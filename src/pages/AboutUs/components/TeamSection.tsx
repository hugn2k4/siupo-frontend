...
const TeamSection: React.FC<BannerProps> = ({ banners, loading }) => {
  ...
  const teamMembers = allChefs.slice(0, 3); // Chỉ lấy 3 người đầu

  const backgroundImage = banners[2]?.url || defaultBackground;

  return (
    <section className="relative bg-primary pt-16 pb-58">
      <div
          ...
          {teamMembers.map((member, index) => (
            <div key={index} className="bg-white rounded-lg shadow-lg text-center overflow-hidden">
              {loading ? (
                ...
                <img src={chefImages[index]} alt={member.name} className="w-full h-64 object-cover" />
              )}
              <div className="p-4">
