            ...
            <button
              ...
              className={`pb-2 font-medium transition-colors ${
                index === 0
                  ...
                  : "text-gray-2 hover:text-green-primary"
              }`}
            >
              {category}
            </button>
          ...
          {menuItems.map((item, index) => (
            <div key={index} className="flex justify-between items-start border-b border-gray-200 pb-6">
              <div className="flex-1">
                <button className="text-xl font-semibold text-gray-800 mb-2 hover:text-green-primary">
                  {item.name}
                </button>
                <p className="text-gray-600 mb-2">{item.description}</p>
                <span className="text-gray-800 font-semibold">{item.calories}</span>
              </div>
              <div className="text-2xl font-bold text-green-primary ml-4">{item.price}</div>
            </div>
          ))}
        </div>

        <div className="text-center mt-12">
          <button className="bg-white text-green-primary border border-green-primary px-8 py-3 font-semibold transition-colors hover:bg-primary hover:text-white">
            View More Menu
          </button>
        </div>
      </div>
